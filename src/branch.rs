...
impl Branch {
    ...
    fn eval_condition(
        ...
        match condition.exec(exec_session, parsed_session) {
            Ok(return_value) => {
                let value =
    ...
    pub fn exec(
        ...
        let condition_result =
            self.eval_condition(&self.main_branch.condition, exec_session, parsed_session)?;

        if condition_result == true {
            return exec_body(&self.main_branch.body, exec_session, parsed_session);
        }

        if !self.else_if_branches.is_empty() {
            for body in &self.else_if_branches {
                let condition_result =
                    self.eval_condition(&body.condition, exec_session, parsed_session)?;
                if condition_result == true {
                    return exec_body(&body.body, exec_session, parsed_session);
                }
            }
        }

        if !self.else_branch.is_empty() {
            return exec_body(&self.else_branch, exec_session, parsed_session);
        }
